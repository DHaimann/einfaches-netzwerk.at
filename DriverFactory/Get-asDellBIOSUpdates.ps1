...
Function Get-asDellBIOSUpdates {
      [CmdletBinding()]
      param (
        [switch]
        $SendMail,
    ...
    BEGIN {
        ...
        $DellCatalogPcXml = Join-Path "$ExtractFolder" "CatalogPC.xml"
        ...
        Write-asLog -Message "Filter BIOS updates from Catalog XML" -LogLevel 1
    ...
    PROCESS {
        foreach ($DellModel in $DellModels) {
            If ($DellUpdateList) {
                ...
                $DellBIOSBrand = $DellUpdateList | Where-Object { $_.SupportedSystems.Brand.Display.'#cdata-section'.Trim() -like "$($DellModel.Brand)" }
                $DellBIOSFullModelName = "$($DellModel.Manufacturer) $($DellModel.Brand) $($DellModel.Model)"
                $DellBIOSFullInformation = $DellBIOSBrand | Where-Object { $_.SupportedSystems.Brand.Model.Display.'#cdata-section'.Trim() -eq "$($DellModel.Model)" }
                $DellBIOSDownloadPath = $DellBIOSFullInformation.path
                $DellBIOSDownloadUrl = "$DellDownloadsUrl" + "$DellBIOSDownloadPath"
                $DellBIOSFileName = $DellBIOSDownloadPath | Split-Path -Leaf
                $DellBIOSVersion = $DellBIOSFullInformation.dellVersion
                $DellBIOSReleaseDate = $DellBIOSFullInformation.releaseDate
                
                # Check package version of download
                Set-Location -Path "$($Global:SiteCode):"
                $PackageFullInformation =""
                $PackageInfoVersion = ""                
                $PackageFullInformation = Get-CMPackage -Name "BIOS - $DellBIOSFullModelName" -Fast
                ...
                If (!($PackageFullInformation -eq '')) {
                    foreach ($PackageInfo in $PackageFullInformation) {
                        ...
                        If ($PackageInfoVersion -eq $DellBIOSVersion) {
                            Write-asLog -Message "ConfigMgr package $PackageInfoPackageID version for $DellBIOSFullModelName is $PackageInfoVersion, up-to-date" -LogLevel 1
                            $ExecuteDownload = $false
                        } Else {                        
                            Write-asLog -Message "ConfigMgr package $PackageInfoPackageID version for $DellBIOSFullModelName is $PackageInfoVersion, update $DellBIOSVersion available" -LogLevel 1
                        }                   
                    }
                } Else {
                    Write-asLog -Message "ConfigMgr package for $DellBIOSFullModelName not found. Will create one." -LogLevel 2
                }

                If ($ExecuteDownload) {
                    # Create download folder per model
                    $DellBIOSDownloadFolder = Join-Path "$DownloadFolder" "$DellBIOSFullModelName\$DellBIOSVersion"
                    If (Test-Path -Path (Split-Path $DellBIOSDownloadFolder)) {
                        Remove-Item -Path "$DellBIOSDownloadFolder" -Recurse -Force -ErrorAction SilentlyContinue
                    }                    
                    $null = New-Item -Path "$DellBIOSDownloadFolder" -ItemType Directory -Force
                    Write-asLog -Message "$DellBIOSDownloadFolder created" -LogLevel 1
                    ...
                    $ExtractFolderfull = Join-Path "$ExtractFolder" "$DellBIOSFullModelName\$DellBIOSVersion"
                    ...
                    Try {
                        Write-asLog -Message "Download $DellBIOSFileName from $DellBIOSDownloadUrl to $DellBIOSDownloadFolder" -LogLevel 1
                        $WebClient.DownloadFile($DellBIOSDownloadUrl, "$DellBIOSDownloadFolder\$DellBIOSFileName")
                        $DownloadToServer = $true
                    }
                    Catch {
                        Write-asLog -Message "Could not download $DellBIOSFileName from $DellBIOSDownloadUrl to $DellBIOSDownloadFolder > skip" -LogLevel 3
                        $DownloadToServer = $false
                    }

                    # Copy files from download folder to extract folder
                    $null = Copy-Item -Path "$DellBIOSDownloadFolder\*" -Destination "$ExtractFolderfull" -Force
                    ...
                    If ($DownloadToServer) {
                        ...
                        $PackageSourceFull = Join-Path "$PackageSourceFull" "$($Global:Settings.FileServerDellBIOSSubfolder)\$DellBIOSFullModelName\$DellBIOSVersion"
                        ...
                        $NewPackageName = "BIOS - $DellBIOSFullModelName"
                        $NewPackageDescription = "Model $($DellModel.ProdCode), Version $DellBIOSVersion, Release Date $DellBIOSReleaseDate"
                        $NewPackageVersion = "$DellBIOSVersion"
                        $NewPackageFolderPath = "$($Global:Settings.CMBIOSPackageFolderPath)\$NewPackageManufacturer"
                           
                        # Create new Package source Path
                        $NewPackageSourcePath = Join-Path $FileServerTargetRoot "$($Global:Settings.FileServerDellBIOSSubfolder)\$DellBIOSFullModelName\$DellBIOSVersion"
...
Get-asDellBIOSUpdates -SendMail -SendTeams
