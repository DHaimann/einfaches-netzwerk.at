...
Function Get-asHPBIOSUpdates {
    ...
    PROCESS {
        foreach ($HPModel in $HPModels) {
            ...
            Write-asLog -Message "Checking $HPModelHPModel product code $HPModelProdCode for BIOS update" -LogLevel 1
            
            # Get Softpaq infos for each Windows 10 version
            foreach ($OSVersion in $OSVersions) {
                Try {
                    $Softpaq = Get-SoftpaqList -platform $HPModelProdCode -os $OS -osver $OSVersion -characteristic SSM -ErrorAction Stop
                ...
                Catch {
                    Write-asLog -Message "Could not retrieve BIOS information for Model $HPModelHPModel Win10 $OSVersion" -LogLevel 2
            ...
            If ($DriverPack) {
                ...
                $PackageInfoVersion = ""                
                $PackageFullInformation = Get-CMPackage -Name "BIOS - $HPModelHPModel" -Fast
                ...
                If ($ExecuteDownload) {               
                    ...
                    $null = Copy-Item -Path "$HPModelDownloadFolder\*" -Destination "$ExtractFolderfull" -Force
                    ...
                    If ($DownloadToServer) {
                        ...
                        $PackageSourceFull = Join-Path "$PackageSourceFull" "$($Global:Settings.FileServerHPBIOSSubfolder)\$HPModelHPModel\$DriverPackVersion"
                        ...
                        $NewPackageName = "BIOS - $HPModelHPModel"
                        ...
                        $NewPackageSourcePath = Join-Path $FileServerTargetRoot "$($Global:Settings.FileServerHPBIOSSubfolder)\$HPModelHPModel\$DriverPackVersion"
...
#Execute Script
Get-asHPBIOSUpdates -SendMail -SendTeams
