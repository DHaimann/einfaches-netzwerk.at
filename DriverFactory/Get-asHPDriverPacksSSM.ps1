...
Function Get-asHPDriverPacks {
    ...
    BEGIN {
        ...
        # Cleanup SSM folder
        $SSMFolder = Join-Path -Path $LocalRootFolder -ChildPath "SSM"
        If (Test-Path -Path $SSMFolder) {
            Remove-Item -Path $SSMFolder -Recurse -Force -ErrorAction SilentlyContinue            
        }
        $null = New-Item -Path $SSMFolder -ItemType Directory -Force
        Write-asLog -Message "$SSMFolder cleaned up" -LogLevel 1
    ...
    PROCESS {
        foreach ($HPModel in $HPModels) {
            ...
            If ($DriverPack) {
				...
				# This is necessary because of multiple driver packages
                ...
                If ($ExecuteDownload) {               
                    ...
                    # Extract Package
                    Start-Process -FilePath $SaveAs -ArgumentList "-e -s -f$($TempFolder)" -Wait
                    ...
                    # Download SSM drivers and use the same version number as for package
                    $aSoftpaq = @()
                    ...
                    Try {
                        ...
                        foreach ($Item in $aSoftpaq) {
                            $SoftpaqId = $Item.Id
                            $SaveAs = "$SSMFolder\$HPModelHPModel\$SoftpaqId.exe"
                            $SaveAsCVA = "$SSMFolder\$HPModelHPModel\$SoftpaqId.cva"
                        ...
                        If (!($null -eq $($Global:Settings.ExtraFiles))) {                    
                           $null = Copy-Item -Path "$($Global:Settings.ExtraFiles)\HPBios\*" -Destination "$SSMFolder\$HPModelHPModel" -Exclude *.bin -Force
                           Write-asLog -Message "Extrafiles copied to $SSMFolder\$HPModelHPModel" -LogLevel 1
                        }

                        $SSMDrivers = $true
                    }
                    Catch {
                        Write-Host "Softpaq-List for $HPModelHPModel not found" -ForegroundColor Red
                        $SSMDrivers = $false
                    }
                    ...
                    If ($SSMDrivers -eq $true) {
                        $PackageSourceFull = "$($SMBShare.LocalPath)" 
                        $PackageSourceFull = Join-Path "$PackageSourceFull" "$($Global:Settings.FileServerHPSSMSubfolder)\$HPModelHPModel\$DriverPackVersion"
                        ...
                        $null = Copy-Item -Path "$SSMFolder\$HPModelHPModel\*" -Destination "$PackageSourceFull" -Container -Force -Recurse
                        Write-asLog -Message "Package files copied to $PackageSourceFull" -LogLevel 1
                        Write-asLog -Message "Will create ConfigMgr package now..." -LogLevel 1
                        ...
                        #Package definition
                        $NewPackageManufacturer = $HPModelManufacturer
                        $NewPackageName = "SSM - $HPModelHPModel"
                        $NewPackageDescription = "Model $HPModelProdCode, Version $DriverPackVersion, Release Date $DriverPackReleaseDate"
                        $NewPackageVersion = "$DriverPackVersion"
                        $NewPackageFolderPath = "$($Global:Settings.CMDriversPackageFolderPath)\$NewPackageManufacturer\SSM"
                           
                        #Create new Package source Path 
                        $NewPackageSourcePath = Join-Path $FileServerTargetRoot "$($Global:Settings.FileServerHPSSMSubfolder)\$HPModelHPModel\$DriverPackVersion"
